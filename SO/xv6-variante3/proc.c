...
typedef struct {
  struct spinlock lock;
  struct proc * vp[NPROC];          /* Vetor de processos */
  int inicio;                       /* Posição do primeiro elemento */
  int fim;                          /* Posição do ultimo elemento */
  int num;                          /* Numero de elementos da fila */
} fila;
...
// Wait for a child process to exit and return its pid and its rtime, wtime and cont.
